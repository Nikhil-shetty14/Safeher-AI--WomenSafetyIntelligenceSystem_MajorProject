...
export default function AlertsPage() {
  ...
  // Normalize coordinates from alert.location. Returns [lat, lng] or null if invalid.
  const normalizeCoords = (loc: any): [number, number] | null => {
    if (!loc) return null;
    ...
    // If lat is outside valid range, maybe lat/lng are swapped — try swap
              ...
              <div style={{ marginTop: "32px", display: "flex", gap: "16px" }}>
                ...
                <button
                  onClick={() => {
                    ...
                    const normalized = normalizeCoords(loc);
                    if (!normalized) {
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((alert, index) => (
            <motion.div
              ...
              className={`glass-card ${alert.status === "active" ? "emergency-glow" : ""}`}
              style={{
                padding: "24px",
                border:
                  alert.status === "active"
